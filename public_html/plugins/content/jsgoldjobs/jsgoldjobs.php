...
class plgContentJSGoldJobs extends JPlugin
{		//for joomla 1.5
		public function onPrepareContent( &$row, &$params, $page=0 )
        {
                if ( JString::strpos( $row->text, 'jsgoldjobs' ) === false ) {
                        return true;
                }

              // expression to search for
                $regex = '/{jsgoldjobs\s*.*?}/i';
        ...
        protected function _process( &$row, &$matches, $count, $regex, $style )
        {
                for ( $i=0; $i < $count; $i++ )
                {
                        $load = str_replace( 'jsgoldjobs', '', $matches[0][$i] );
        ...
        protected function _load( $position, $style=-2 )
        {
				...
				$result = $model->getGoldJobs($noofjobs,$theme);
				$jobs = $result[0];
				$trclass = $result[1];	
				$dateformat = $result[2];	
							...
							$isodd = 1;
									...
									foreach($jobs as $job)	{ 
										$contents .=  '<tr id="mc_field_row" class="'.$trclass[$isodd].'">';
													...
													$contents .=   $job->title.'</a></span></td>';																															//$job->jobcategory
...
*/
						...
						if ($sliding == 1) {
							for ($a = 0; $a < $consecutivesliding; $a++){
								...
								<script type="text/javascript" language=Javascript>
									jQuery(document).ready(function(){
										jQuery("marquee#plg_jsgoldjobs").marquee("pointer").mouseover(function () {
							...
							$contents =  '<marquee id="plg_jsgoldjobs"  direction="up" scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
...
*/
						...
						if ($sliding == 1) {
							...
							$contents =  '<marquee id="plg_jsgoldjobs"  scrollamount="2" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
						}
						//sce
