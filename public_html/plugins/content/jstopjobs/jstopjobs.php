        ...
        protected function _load( $position, $style=-2 )
        {
				...
				if (isset($jobs)) { 
					...
					foreach ($jobs as $job) {
						...
						if ($company == 1) $contents .=  '<small>'.$companytitle.': <span id="themeanchor"><a class="anchor" href="index.php?option=com_jsjobs&c=jsjobs&view=employer&layout=view_company&vm=5&md=' .$job->companyaliasid . '&jobcat=' .$job->jobcategory . '&Itemid='.$itemid.'">'.$job->companyname.'</a></span></small><br />';	
				...
				if ($sliding == 1) {                    
					...
					if($slidingdirection == 1 ){
						...
						$contents =  '<marquee id="plg_topjsjobs"  direction="up" scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
						...
						$contents =  '<marquee  id="plg_topjsjobs" scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
					}	
					$contents .= '
					<script type="text/javascript" language=Javascript>
						jQuery(document).ready(function(){
							jQuery("marquee#plg_topjsjobs").marquee("pointer").mouseover(function () {
