...
class plgContentJSJobcategories extends JPlugin
{
	// for joomla 1.5	
	public function onPrepareContent( &$row, &$params, $page=0 )  {
                if ( JString::strpos( $row->text, 'jsjobcategories' ) === false ) {
                        return true;
		}
              // expression to search for
                $regex = '/{jsjobcategories\s*.*?}/i';
        ...
        protected function _process( &$row, &$matches, $count, $regex, $style )  {
                for ( $i=0; $i < $count; $i++ )
                {
                        $load = str_replace( 'jsjobcategories', '', $matches[0][$i] );
		...
		$contents = '';
		$noofcategories = $this->params->get('noofcategories', 7);
		$jobsincategory =$this->params->get('jobsincategory', 1);
		$allcategories = $this->params->get('allcategories', 0);
		...
		if ($sliding == 1) {                    
			...
			if($slidingdirection == 1 ){
				...
				$contents =  '<marquee id="plg_jsjobcategories"  direction="up" scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
				...
				$contents =  '<marquee id="plg_jsjobcategories"  scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
			}	
		$contents .= '
		<script type="text/javascript" language=Javascript>
			jQuery(document).ready(function(){
				jQuery("marquee#plg_jsjobcategories").marquee("pointer").mouseover(function () {
