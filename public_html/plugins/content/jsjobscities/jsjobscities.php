...
class plgContentJSJobsCities extends JPlugin
{
	// for joomla 1.5
	public function onPrepareContent( &$row, &$params, $page=0 )  {
                if ( JString::strpos( $row->text, 'jsjobscities' ) === false ) {
                        return true;
		}
              // expression to search for
                $regex = '/{jsjobscities\s*.*?}/i';
       ...
       public function onContentPrepare( $context, &$row, &$params, $page=0 ){
        ...
        protected function _process( &$row, &$matches, $count, $regex, $style )  {
                for ( $i=0; $i < $count; $i++ )
                {
                        $load = str_replace( 'jsjobscities', '', $matches[0][$i] );
		...
		$showonlycityhavejobs = $this->params->get('schj', 1);
		...
		if ($cities) { 
			...
			if ($sliding == 1) {                    
				...
				if($slidingdirection == 1 ){
					...
					$contents =  '<marquee id="plg_jsjobscity"  direction="up" scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
					...
					$contents =  '<marquee id="plg_jsjobscity"  scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
				}	
				$contents .= '
				<script type="text/javascript" language=Javascript>
					jQuery(document).ready(function(){
						jQuery("marquee#plg_jsjobscity").marquee("pointer").mouseover(function () {
