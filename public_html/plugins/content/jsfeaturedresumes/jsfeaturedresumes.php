...
class plgContentJSFeaturedResumes extends JPlugin
{		// for joomla 1.5
		public function onPrepareContent( &$row, &$params, $page=0 )
        {
                if ( JString::strpos( $row->text, 'jsfeaturedresumes' ) === false ) {
                        return true;
                }

              // expression to search for
                $regex = '/{jsfeaturedresumes\s*.*?}/i';
        ...
        protected function _process( &$row, &$matches, $count, $regex, $style )
        {
                for ( $i=0; $i < $count; $i++ )
                {
                        $load = str_replace( 'jsfeaturedresumes', '', $matches[0][$i] );
        ...
        protected function _load( $position, $style=-2 )
        {
				...
				$result = $model->getFeaturedResumes($noofresumes,$theme);
				$resumes = $result[0];
				$trclass = $result[1];	
				$dateformat = $result[2];	
				/** sce */
					...
					if($listingstyle == 1){ //vertical listing
						...
						$contents =  '<table cellpadding="0" cellspacing="0" border="0" width="100%" class="contentpane">';
												...
												$r_l=JRoute::_('index.php?option=com_jsjobs&c=jsjobs&view=jobseeker&layout=view_resume&vm=3&rd='. $resume->aliasid . '&Itemid='.$itemid);		
													...
													if ($showlocation == 1){
														$contents .=  '<td >';
															...
															if(isset($location))
																$contents .= '<small>'.$locationtitle.': '.$location.'</small>';
														$contents .= '</td>';
													}	
							...
							if ($sliding == 1) {
								for ($a = 0; $a < $consecutivesliding; $a++){
									...
									<script type="text/javascript" language=Javascript>
										jQuery(document).ready(function(){
											jQuery("marquee#plg_jsfeaturedresume").marquee("pointer").mouseover(function () {
								...
								$contents =  '<marquee id="plg_jsfeaturedresume"  direction="up" scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
					   ...
					   $contents = '<table cellpadding="0" cellspacing="0" border="0" width="100%" class="contentpane">';
						...
						if ($sliding == 1) {
							...
							$contents =  '<marquee  id="plg_jsfeaturedresume" scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
						}
						//sce
