        ...
        protected function _load( $position, $style=-2 )
        {
				...
				if (isset($jobs)) { 
					...
					foreach ($jobs as $job) {
						...
						if ($company == 1) $contents .=  '<small>'.$companytitle.': <span id="themeanchor"><a class="anchor" href="index.php?option=com_jsjobs&c=jsjobs&view=employer&layout=view_company&vm=5&md=' .$job->companyid . '&Itemid='.$itemid.'">'.$job->companyaliasid.'</a></span></small><br />';	
						if ($category == 1) $contents .= '<small>'.$categorytitle.': '.$job->cat_title.'</small><br />';	
						if ($subcategory == 1) $contents .= '<small>'.$subcategorytitle.': '.$job->cat_title.'</small><br />';	
				...
				if ($sliding == 1) {                    
					...
					if($slidingdirection == 1 ){
						...
						$contents =  '<marquee id="plg_newestjsjobs"  direction="up" scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
						...
						$contents =  '<marquee id="plg_newestjsjobs"  scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
					}	
					$contents .= '
					<script type="text/javascript" language=Javascript>
						jQuery(document).ready(function(){
							jQuery("marquee#plg_newestjsjobs").marquee("pointer").mouseover(function () {
				...
				//sce
                	$viewall = '<div id="viewall" width="100%" align="center"><span id="themeanchor"><a id="button" class=" button minpad" href="index.php?option=com_jsjobs&c=jsjobs&view=jobseeker&layout=listnewestjobs&Itemid='.$itemid.'">'.JText::_('JS_VIEW_ALL').'</a></span></div>';
                        $contents .=$viewall;
               return $contents;
        }
}
