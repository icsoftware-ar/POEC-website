...
class plgContentJSJobsStates extends JPlugin
{   // for joomla 1.5

	public function onPrepareContent( &$row, &$params, $page=0 )  {
                if ( JString::strpos( $row->text, 'jsjobsstates' ) === false ) {
                        return true;
		}
              // expression to search for
                $regex = '/{jsjobsstates\s*.*?}/i';
       ...
       public function onContentPrepare( $context, &$row, &$params, $page=0 ){
        ...
        protected function _process( &$row, &$matches, $count, $regex, $style )  {
                for ( $i=0; $i < $count; $i++ )
                {
                        $load = str_replace( 'jsjobsstates', '', $matches[0][$i] );
		...
		$noofrecord = $this->params->get('noofjobs', 7);
		if($noofrecord>100) $noofrecord=100;
		
		$showonlystatehavejobs = $this->params->get('ssthj', 1);
		...
		$result = $model->getJobsStates($showonlystatehavejobs,$theme,$noofrecord);
		$states = $result[0];
		$trclass = $result[1];
		$contents = '<table cellpadding="0" cellspacing="0" border="0" width="100%" class="contentpane">';
			...
			if ($states ) { 
				foreach ($states as $state) {
						...
						$lnks = 'index.php?option=com_jsjobs&c=jsjobs&view=jobseeker&layout=listnewestjobs&state='. $state->stateid .'&lt=1&Itemid='.$itemid; 
						$lnks = JRoute::_($lnks);
						$contents .= '<td width="'.$colwidth.'"><span id="themeanchor"><a class="anchor" href="'.$lnks.'" >'.$state->statename;
								$contents .=' ('. $state->totaljobsbystate.')';
								$contents .= '</a></span></td>';
			...
			if ($sliding == 1) {                    
				...
				if($slidingdirection == 1 ){
					...
					$contents =  '<marquee id="plg_jsjobsstates"  direction="up" scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
					...
					$contents =  '<marquee id="plg_jsjobsstates"  scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
				}	
				$contents .= '
				<script type="text/javascript" language=Javascript>
					jQuery(document).ready(function(){
						jQuery("marquee#plg_jsjobsstates").marquee("pointer").mouseover(function () {
