...
Joomla.submitbutton = function(task){
        if (task == ''){
                ...
                if (task == 'savejobagesave' || task == 'savejobageandnew' || task == 'savejobage' ){
...
function submitbutton(pressbutton) {
	...
	if (pressbutton == 'savejobagesave' || pressbutton == 'savejobageandnew' || pressbutton == 'savejobage' ){
		returnvalue = validate_form(document.adminForm);
	}else returnvalue  = true;
...
<table width="100%" >
	...
	<tr>
		...
		<input type="submit" class="button" name="submit_app" onclick="return validate_form(document.adminForm)" value="<?php echo JText::_('JS_SAVE'); ?>" />
			...
			<input type="hidden" name="task" value="savejobage" />
