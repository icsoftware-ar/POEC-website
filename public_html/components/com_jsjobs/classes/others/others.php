...
class JSJobsothers
{
	...
	function generateRequest($orderid){
		...
		$order = $orderresult[0];
		$otherlink = $orderresult[1]->paymentlink;
		$action = $otherlink;
		if(!empty($order)){
		?>
...
<?php
	...
	function getOrderById($orderid){
		...
		if(!empty($result)){
			...
			$otherlink = $this->getPaymentMethodLinkByPackageid($result->packageid,$result->packagefor);
			$return[0] = $result;
			$return[1] = $otherlink;
			return $return;
	...
	function getPaymentMethodLinkByPackageid($packageid,$packagefor){
		...
		$query = "SELECT paymentlink.link AS paymentlink
					...
					WHERE paymentmethod.paymentmethod = 'other' AND paymentlink.packageid = ".$packageid." AND paymentlink.packagefor=".$packagefor;
		$db->setQuery($query);
		$result = $db->loadObject();
		return $result;
	}
	function loadConfig(){
		$db = JFactory::getDBO();
		$query = "SELECT payment.* FROM `#__js_job_paymentmethodconfig` AS payment WHERE payment.configfor = 'others'";
		$db->setQuery($query);
