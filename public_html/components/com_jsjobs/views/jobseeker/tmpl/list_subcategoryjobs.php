 ...
 */
 ...
 $document->addStyleSheet('components/com_jsjobs/themes/'.$this->config['theme']);
	$link = "index.php?option=com_jsjobs&c=jsjobs&view=jobseeker&layout=list_subcategoryjobs&jobsubcat=". $this->jobsubcat."&Itemid=".$this->Itemid;
	if(isset($this->jobs[0]->aliasid))$link = "index.php?option=com_jsjobs&c=jsjobs&view=jobseeker&layout=list_subcategoryjobs&jobsubcat=". $this->jobs[0]->aliasid."&Itemid=".$this->Itemid;

	$cm = '';
		$ptitle = '';
		if (isset($_GET['cn'])) $cn=$_GET['cn']; else $cn='';
		$catlink = JRoute::_("index.php?option=com_jsjobs&c=jsjobs&view=jobseeker&layout=list_jobs&jobcat=". $this->jobs[0]->cat_id."&Itemid=".$this->Itemid);
		...
		<div id="tp_header" <?php if($this->config['topimage'] == 0) echo 'style="background:none;"';?>>
			...
			<span id="tp_curloc">
				<?php if ($this->config['cur_location'] == 1) {
						echo JText::_('JS_CUR_LOC'); ?> : <a href="<?php echo $jobcatlink; ?>" class="curloclnk"><?php echo JText::_('JS_JOB_CATEGORIES'); ?></a> > <a href="<?php echo $catlink; ?>" class="curloclnk"><?php echo JText::_('JS_JOBS_LIST_BY_CATEGORY'); ?></a> > <?php echo JText::_('JS_SUB_CATEGORY');
				} ?>
			</span>
		</div>
		<div id="tp_links">
...
<?php 
		...
		<?php foreach($this->goldjobs as $job) {
					...
					<div id="jl_maindiv" class="<?php echo $tdclass[$isodd];?>">
						<div id="jl_leftdiv">
								<?php 
									$link = 'index.php?option=com_jsjobs&c=jsjobs&view=employer&layout=view_job&vj=2&oi='.$job->jobaliasid.'&Itemid='.$this->Itemid;
								?>
								...
								<div id="jl_button">
									<?php 
										$link = 'index.php?option=com_jsjobs&c=jsjobs&view=jobseeker&layout=job_apply&aj=5&bi='.$job->jobaliasid.'&jobcat='.$this->jobs[0]->cat_id.'&Itemid='.$this->Itemid; 
										?>
	...
	<?php $querystring='&jobsubcat='.$this->jobs[0]->aliasid.'&Itemid='.$this->Itemid ?>				
	<form action="<?php echo JRoute::_('index.php?option=com_jsjobs&c=jsjobs&view=jobseeker&layout=list_subcategoryjobs'); ?>" method="post">
