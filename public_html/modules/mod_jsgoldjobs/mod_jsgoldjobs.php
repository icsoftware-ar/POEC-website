...
if ($jobs) { 
	...
	if($listingstyle == 1){ //vertical listing
		...
		for ($a = 0; $a < $consecutivesliding; $a++){
			...
			<script type="text/javascript" language=Javascript>
				jQuery(document).ready(function(){
					jQuery("marquee#mod_jsgoldjobs").marquee("pointer").mouseover(function () {
		...
		$contents =  '<marquee id="mod_jsgoldjobs"  direction="up" scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
		$contents = $top.$contents.'<br clear="all">';
		}

		echo $contents ;
		
	}else{ // horizontal listing   //scs pending 
	   $contents = '<table cellpadding="0" cellspacing="0" border="0" width="100%" class="contentpane">';
		$isodd = 1;
		$count = 1;
		if ($shtitle == 1){
		...
		if($sliding == 1){
			...
			$contents =  '<marquee id="mod_jsgoldjobs"  scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
		}
