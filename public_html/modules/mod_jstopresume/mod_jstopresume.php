 ...
 * Created on:	Oct 5, 2009
 ^
 + Project: 		JS Jobs 
 * File Name:	module/topjsresume.php
 ^ 
...
$result = $model->getTopResumes($noofresumes,$theme);
$resumes = $result[0];
$trclass = $result[1];	
$dateformat = $result[2];	
/** sce */
...
if ($resumes) { 
	...
	foreach ($resumes as $resume) {
		...
		if ($title == 1) {  if($colcount == $colperrow) { $contents .= '</tr><tr id="mc_field_row" class="'.$trclass[$isodd].'">'; $colcount = 0; } $colcount++; 
			...
			$contents .= '<td width="'.$colwidth.'">';  if(isset($resume->nationality))$contents .=  '<small>'.JText::_('JS_NATIONALITY').': '.$resume->nationality.'</small></td>';	
	...
	if ($sliding == 1) {
		...
		if($slidingdirection == 1 ){
			...
			$contents =  '<marquee id="mod_jstopresume"  direction="up" scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
			...
			$contents =  '<marquee id="mod_jstopresume" scrollamount="1" onmouseover="this.stop();" onmouseout="this.start()";>'.$contents.'</marquee>';
		}
		$contents .= '
		<script type="text/javascript" language=Javascript>
			jQuery(document).ready(function(){
				jQuery("marquee#mod_jstopresume").marquee("pointer").mouseover(function () {
